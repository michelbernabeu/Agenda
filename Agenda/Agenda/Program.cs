...
namespace MenuAgenda
{
    ...
    {   
        ...
        static string ValidarDni(string dni)
        {
            Console.Clear();
            bool dniValid = false;
            int lletranumero = Convert.ToInt32(dni.Substring(0,8));
            lletranumero = lletranumero % 23;
            string lletra = " ";
            if (lletranumero == 1) lletra = "R";
            if (lletranumero == 2) lletra = "W";
            if (lletranumero == 3) lletra = "A";
            if (lletranumero == 4) lletra = "G";
            if (lletranumero == 5) lletra = "M";
            if (lletranumero == 6) lletra = "Y";
            if (lletranumero == 7) lletra = "F";
            if (lletranumero == 8) lletra = "P";
            if (lletranumero == 9) lletra = "D";
            if (lletranumero == 10) lletra = "X";
            if (lletranumero == 11) lletra = "B";
            if (lletranumero == 12) lletra = "N";
            if (lletranumero == 13) lletra = "J";
            if (lletranumero == 14) lletra = "Z";
            if (lletranumero == 15) lletra = "S";
            if (lletranumero == 16) lletra = "Q";
            if (lletranumero == 17) lletra = "V";
            if (lletranumero == 18) lletra = "H";
            if (lletranumero == 19) lletra = "L";
            if (lletranumero == 20) lletra = "C";
            if (lletranumero == 21) lletra = "K";
            if (lletranumero == 22) lletra = "E";
            if (lletranumero == 0) lletra = "T";
            while (!dniValid)
            {
                var dniRegex = new Regex(@"^[1-9][0-9]{7}[A-Z]$");
                if (!dniRegex.IsMatch(dni))
                {
                    Console.WriteLine("Aquest DNI es invalid, escriu un altre");
                    dni = Console.ReadLine();
                }
                else if (lletra == dni.Substring(8, 1)) dniValid = true;
                else
                {
                    dniValid = false;
                    Console.WriteLine("Aquest DNI es invalid, escriu un altre");
        ...
        static string ValidarCorreu(string correu)
        {
            ...
            while (!correuValid)
            {
                var correuRegex = new Regex(@"^[a-zA-Z0-9]+@[a-zA-Z]{3,}\.(com|es|cat)$");
                if (!correuRegex.IsMatch(correu))
                {
                    Console.WriteLine("Correu invalid, escriu un altre.");
